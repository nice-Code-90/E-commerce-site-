...
const Cart = ({ history }) => {
    ...
    const { cartItems } = useSelector(state => state.start)
    ...
    return (
        <Fragment>
            <MetaData title={'Yout cart'}></MetaData>
                                                ...
                                                <div className="stockCounter d-inline">
                                                    ...
                                                    <span className="btn btn-primary plus" onClick={
                                                        () => increaseQty(item.product,
                                                            item.quantity, item.stock)}>+</span>
                                                </div>
                                            </div>

                                            <div className="col-4 col-lg-1 mt-4 mt-lg-0">
                                                <i id="delete_cart_item" className="fa fa-trash btn btn-danger" onClick={() =>
                                                    removeCartItemHandler(item.product)}></i>
                                            </div>
                        ...
                        <div className="col-12 col-lg-3 my-4">
                            <div id="order_summary">
                                ...
                                <p>Subtotal:  <span className="order-summary-values">{cartItems.reduce((acc, item) =>
                                    (acc + Number(item.quantity)), 0)}</span></p>
